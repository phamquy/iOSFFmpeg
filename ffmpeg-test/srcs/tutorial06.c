...
/* Add or subtract samples to get a better sync, return new
 audio buffer size */
int synchronize_audio(VideoState *vidState, // big struct
                      short *samples,       // pointer to samples buffer
                      int samples_size,     // size of buffer
                      double pts)
{
    int n;
    double ref_clock;
    ...
    if(vidState->av_sync_type != AV_SYNC_AUDIO_MASTER) {
        ...
        // ???: what diff is negative, and its abs value is greater than AC_NOSYNC_THRESHOLD
