    ...
    for(;;) {
        while(vidState->audio_pkt_size > 0) {
            ...
            *pts_ptr = pts;
            ...
            // (in this case 16bits ~ 2bytes per sample)
...
void video_refresh_timer(void *userdata) {
    ...
    if(vidState->video_st) {
        ...
        // If there is no picture in queue, wait for it.
        if(vidState->pictq_size == 0) {
            schedule_refresh(vidState, 1);
        } else {
            vidPic = &vidState->pictq[vidState->pictq_rindex];
            
            // Calculate time has lapsed since last pts
            delay = vidPic->pts - vidState->frame_last_pts; /* the pts from last time */
            ...
            vidState->frame_last_pts = vidPic->pts;
            
            /* update delay to sync to audio */
            ref_clock = get_audio_clock(vidState);
            diff = vidPic->pts - ref_clock;
...
/**
 (in) vidState   : the big struct
 (in) src_frame  : decoded frame, which may or may not contain pts information
 (in/out) pts    : the pts that the frame should take
 */
double synchronize_video(VideoState *vidState, AVFrame *src_frame, double pts) {
    ...
    frame_delay = av_q2d(vidState->video_st->codec->time_base);  // ???: Why using codec's timebase
...
int our_get_buffer(struct AVCodecContext *c, AVFrame *pic) {
    ...
    printf("\n1st pkt's pts: %lld", global_video_pkt_pts);
...
int video_thread(void *arg) {
    ...
    for(;;) {
        ...
        if (  packet->dts == AV_NOPTS_VALUE                     /// there is no packet's dts
           && pFrame->opaque                                    /// but we have first pkt's pts
           && *(uint64_t*)pFrame->opaque != AV_NOPTS_VALUE)     /// and it is valid.
        {
            pts = *(uint64_t *)pFrame->opaque;
            
        }
        else if(packet->dts != AV_NOPTS_VALUE)                  /// if we have packet's dts --> use it
        {
            printf("\nPkt's dts: %lld",  packet->dts);
            pts = packet->dts;
        
        } else {            /// otherwise, set it to zero pts
            pts = 0;
        }
        /// ???: why using video stream timebase here? 
