...
typedef struct VideoState {
    ...
    // the buffer that to store decode audio frames
    // used in: decode, and audio synchronize function
...
int synchronize_audio(VideoState *vidState, short *samples,
    ...
    if(vidState->av_sync_type != AV_SYNC_AUDIO_MASTER) {
        ...
        printf("A-R diff: %f", ref_clock);
        
        if(diff < AV_NOSYNC_THRESHOLD) {
            
            // accumulate the diffs
            vidState->audio_diff_cum =
                diff + vidState->audio_diff_avg_coef * vidState->audio_diff_cum;
            
            
            
            if(vidState->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
                vidState->audio_diff_avg_count++;
                printf("\nAudio dif avg count: %d", vidState->audio_diff_avg_count);
            } else {
...
/**
 Decode audio pkts from queue and fill the result in audio_buf (that have size of buf_size) 
...
int audio_decode_frame(VideoState *vidState, uint8_t *audio_buf, int buf_size, double *pts_ptr) {
    ...
    for(;;) {
        while(vidState->audio_pkt_size > 0) {
            ...
            data_size = buf_size;
            len1 = avcodec_decode_audio4(vidState->audio_st->codec,
                                         decoded_aframe,
                                         &got_frame,
                                         &pktTemp);
            ...
            if (got_frame) {
                ...
                printf("\nFrame timestamp: %f", decoded_aframe->pts);
...
int queue_picture(VideoState *vidState, AVFrame *pFrame, double pts) {
    ...
    // index vidState set to 0 initially
    vp = &vidState->pictq[vidState->pictq_windex];
...
int decode_thread(void *arg) {
    ...
    int errCode = avio_open2(&pFormatCtx->pb, vidState->filename, AVIO_FLAG_READ, &pFormatCtx->interrupt_callback, NULL);
    char errString[1024];
    memset(errString, 0, 1024);
    av_strerror(errCode, errString, 1024);
    printf("Open io error: %s", errString);
    if (errCode)
        return -1;
    ...
    if(audio_index >= 0) {
        stream_component_open(vidState, audio_index); // <-- Audio decode thread is created in here
    }
    if(video_index >= 0) {
        stream_component_open(vidState, video_index); // <-- Video decode thread is created in here
    }
    ...
    for(;;) {
        ...
        if(vidState->seek_req) {
            int stream_index= -1;
            int64_t seek_target = vidState->seek_pos;
            
            if     (vidState->videoStream >= 0) stream_index = vidState->videoStream;
            else if(vidState->audioStream >= 0) stream_index = vidState->audioStream;
            ...
            if(!av_seek_frame(vidState->pFormatCtx,
                              stream_index,
                              seek_target,
                              vidState->seek_flags)) {
                fprintf(stderr, "%s: error while seeking\n", vidState->pFormatCtx->filename);
            } else {
                if(vidState->audioStream >= 0) {
                    packet_queue_flush(&vidState->audioq);
                    packet_queue_put(&vidState->audioq, &flush_pkt);
                }
                if(vidState->videoStream >= 0) {
                    packet_queue_flush(&vidState->videoq);
                    packet_queue_put(&vidState->videoq, &flush_pkt);
                }
            }
            vidState->seek_req = 0;
        }
        ...
        // Limit the size that queue can have
...
void stream_seek(VideoState *vidState, int64_t pos, int rel) {
    
    if(!vidState->seek_req) {
        vidState->seek_pos = pos;
        vidState->seek_flags = rel < 0 ? AVSEEK_FLAG_BACKWARD : 0;
        vidState->seek_req = 1;
    }
}
